...
exppath = '/home/scw9/wray_workspace/Savant_Arabic/new_ica/savant_main' # ENTER PATH TO EXPERIMENT FODLER HERE (folder containing meg and mri folders)
megpath = exppath + '/meg'
mripath = exppath + '/mri'
os.chdir(exppath)
expname = 'SavantAra' #'TarkAra' # ENTER EXPERIMENT CODENAME HERE (e.g., SAVANTARA or whatever convention we follow)
# pphelpers has all the functions that I use for preprocessing
from pphelpers import *

# Get a list of all the subjects

#subjlist = ['Y0321','Y0367','Y0368','Y0369','Y0373','Y0374','Y0375','Y0377','Y0378','Y0379','Y0381','Y0382','Y0383','Y0387','Y0388','Y0393','Y0394']
subjlist = ['Y0374b']
#,'Y0369','Y0373','Y0374','Y0375','Y0377','Y0378','Y0379','Y0381','Y0382','Y0383','Y0387','Y0394']
#notark: 'Y0376'
#'Y0321','Y0367','Y0368','Y0369','Y0373','Y0374','Y0375','Y0377','Y0378','Y0379','Y0381','Y0382','Y0383','Y0387','Y0388','Y0393'
#subj = 'Y0369' ##IN CASE YOU'RE RUNNING MORE SUBJECTS AT ONCE, DON'T RUN THIS
...
tmin = -0.2  # INSERT TIME TO INCLUDE BEFORE TRIGGER (baseline) in SECONDS
...
for subj in subjlist: #DON'T RUN THIS LINE IF YOU ARE RUNNING ONLY ONE SUBJECT
    ...
    while done == False:
        ...
        if done == True:
            ...
            curr_ica.save('%s/%s/%s_%s_sol-ica.fif' % (megpath, subj, subj, expname))
        ...
        # If you're not done (you want to exclude bad channels or epochs)
            ...
            if redobads.lower() == 'y':
                ...
                # If bad channels have changed (you selected new ones, say)
            ...
            # If you want to drop epochs, it is best to do it automatically (based on the variance calculation above)
            ...
            if epochdrop.lower() == 'm':
                ...
                epochs4ica.plot(block = True, n_channels = 40)
...
for subj in subjlist: ##DON'T RUN THIS LINE IF YOU ARE RUNNING ONLY ONE SUBJECT
    ...
    fname = '%s/%s/%s_%s_butterfly.svg' % (megpath, subj, subj, expname)
    fig_evo.savefig(fname)
    fname = '%s/%s/%s_%s_butterfly.png' % (megpath, subj, subj, expname)
    fig_evo.savefig(fname, dpi = 200)

    # Compute noise covariance matrix based on baseline
    noise_cov = mne.compute_covariance(epochs, tmax = 0.0, method = ['shrunk', 'diagonal_fixed', 'empirical'])#method = 'auto')
    mne.write_cov('%s/%s/%s_%s-cov.fif' % (megpath, subj, subj, expname), noise_cov)
    ...
    fname = '%s/%s/%s_%s_covmat.svg' % (megpath, subj, subj, expname)
    fig_covmat.savefig(fname)
    fname = '%s/%s/%s_%s_covmat.png' % (megpath, subj, subj, expname)
    fig_covmat.savefig(fname, dpi = 200)
    fname = '%s/%s/%s_%s_covnoise.svg' % (megpath, subj, subj, expname)
    fig_covnoise.savefig(fname)
    fname = '%s/%s/%s_%s_covnoise.png' % (megpath, subj, subj, expname)
    fig_covnoise.savefig(fname, dpi = 200)

    fig_white = evoked.plot_white(noise_cov, time_unit = 's')
    fname = '%s/%s/%s_%s_whitened.svg' % (megpath, subj, subj, expname)
    fig_white.savefig(fname)
    fname = '%s/%s/%s_%s_whitened.png' % (megpath, subj, subj, expname)
