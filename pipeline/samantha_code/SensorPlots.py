...
for subj in subjects:
        ...
        epochs = mne.read_epochs('meg/%s/%s_%s_1-40-ica-rej-epo.fif' %(subj,subj, expName))
...
# Plot average evoked by participant
        if indButterfly:
            ...
            evoked_plot.savefig('Plots/Individuals/%s/%s_evoked.png'%(subj,subj))
        ...
        if indRegionPlots:
            ...
            for region in regions:
                ...
                sensorPlot[0].savefig('Plots/Individuals/%s/%s_%s_avgSensorPlot.png' %(subj,subj,region))

if avgButterfly:
    all_evokeds = mne.combine_evoked(allEpos, weights='equal')
    evoked_plot = all_evokeds.plot_joint(show=False, title = 'Average')
    evoked_plot.savefig('Plots/Group/%s_avg_evoked_n%s.png' %(date,str(sampleSize)))
...
if avgRegionPlots:
    ...
    for region in regions:
        sensorPlot = mne.viz.plot_compare_evokeds(grandAvgsDict,
            ...
            show = False)
        sensorPlot[0].set_figwidth(25)
        sensorPlot[0].savefig('Plots/Group/%s_avg_%s_evoked_n%s.png' %(date, region, str(sampleSize)))
