...
subjlist = ['Y0312','Y0321','Y0367','Y0368','Y0369','Y0371','Y0373','Y0374','Y0378','Y0379','Y0381','Y0382','Y0387','Y0393','Y0395','Y0396']
#['Y0119','Y0208','Y0366','Y0371','Y0372','Y0395','Y0396']
...
for subj in subjlist: ##DON'T RUN THIS LINE IF YOU ARE RUNNING ONLY ONE SUBJECT
    ...
    noise_cov = mne.compute_covariance(allepochs, tmax = 0.0, method = ['shrunk', 'diagonal_fixed', 'empirical'])#method = 'auto') #replace method with auto to have additional option
    #noise_cov = mne.compute_covariance(tarkepochs, tmax = 0.0, method = ['shrunk', 'diagonal_fixed', 'empirical'])#method = 'auto')
    mne.write_cov('%s/%s/%s_%s-cov_all_epochs.fif' % (megpath, subj, subj, expname), noise_cov)
    ...
    fname = '%s/%s/%s_%s_covmat_all_epochs.svg' % (megpath, subj, subj, expname)
    fig_covmat.savefig(fname)
    fname = '%s/%s/%s_%s_covmat_all_epochs.png' % (megpath, subj, subj, expname)
    fig_covmat.savefig(fname, dpi = 200)
    fname = '%s/%s/%s_%s_covnoise_all_epochs.svg' % (megpath, subj, subj, expname)
    fig_covnoise.savefig(fname)
    fname = '%s/%s/%s_%s_covnoise_all_epochs.png' % (megpath, subj, subj, expname)
    fig_covnoise.savefig(fname, dpi = 200)

    fig_white = evoked.plot_white(noise_cov, time_unit = 's')
    fname = '%s/%s/%s_%s_whitened_all_epochs.svg' % (megpath, subj, subj, expname)
    fig_white.savefig(fname)
    fname = '%s/%s/%s_%s_whitened_all_epochs.png' % (megpath, subj, subj, expname)
