...
subjects = ['Y0119', 'Y0208', 'Y0312', 'Y0321', 'Y0367', 'Y0368', 'Y0369', 'Y0371', 'Y0373', 'Y0374', 'Y0378', 'Y0379', 'Y0381', 'Y0387', 'Y0393', 'Y0395', 'Y0396', 'Y0400']
#subjects = ['Y0119','Y0208','Y0312','Y0321','Y0367','Y0368','Y0369','Y0371','Y0373','Y0374','Y0378','Y0379','Y0381','Y0382','Y0387','Y0393','Y0395','Y0396','Y0400']
#subjects = [,'Y0371']mne.set_log_level(verbose='WARNING')
eelbrain.configure(frame=False)
...
output = os.path.join('new_baseline/savant_main', 'Plots/Group/%s_Regressions_n%s/' %(date,str(sampleSize)))
...
#('M350 Analysis', (0.2, 0.5), ['LOBE.TEMPROAL-lh'], 'left temporal lobe', 'PALS_B12_Lobes', 'left', 'stemFrequency + wholeWordFreq', True, True, False),
    ...
    for subj in subjects:
        ...
        regResFileName = op.join('new_baseline/savant_main/','Regressions/',subj,'Tark',hemi,regionName,str(time[0])+'-'+str(time[1]),formula+'.pickle')
...
#                logFile = open(op.join('Logs','%s-Savant_Ara.log' %subj))
            ...
            if not os.path.exists(op.join('new_baseline/savant_main/','Regressions',subj,'Tark',hemi,regionName,str(time[0])+'-'+str(time[1]))):
                os.makedirs(op.join('new_baseline/savant_main/','Regressions',subj,'Tark',hemi,regionName,str(time[0])+'-'+str(time[1])))
...
#            xlim = (epoch_tmin, epoch_tmax),
        ...
        #plt.plot(int_timecourse.mean(dims='case'),'black',label=str(coef.split(':')[0]),linewidth=0.75,linestyle='dashed')
        plt.plot(int_timecourse.mean(dims='case'),'red',label=str('symbol'),linewidth=0.75)
        plt.plot(effect_timecourse.mean(dims='case'),'blue',label=str(coef.split(':')[0]),linewidth=0.75)
        ...
        if ':' in coef:
            fig.suptitle("Intercept vs. Coefficient for %s in %s cluster, %s - %s ms" %(str(coef.split(':')[0]),regionName,tStartName,tStopName))
        else:
            fig.suptitle("Intercept vs. Coefficient for %s in %s cluster, %s - %s ms" %(coef,regionName,tStartName,tStopName))
