...
main_exp_dir = '/home/scw9/wray_workspace/tagsurp_mne/'
...
for s in subjects:
	subject = s
	subject1 = subject
	ica = ICA(n_components=0.95,method='fastica',random_state=42,max_iter=30000)
	#ica = ICA(n_components=0.95,method='fastica',random_state=42)
	raw = mne.io.read_raw_fif(main_exp_dir + 'meg/%s/%s_TagSurp_NR-raw.fif' %(subject,subject1),preload=True)
	raw.filter(l_freq=0.1,h_freq=40,method='iir')
	ica.fit(raw,reject = dict(mag=3e-12))
	#ica.fit(raw)
	ica.plot_sources(raw)
        #raw_input('Press enter to continue')   #python 2
	input('Press enter to continue')  #python 3
	raw=ica.apply(raw,exclude=ica.exclude)
	raw.save(main_exp_dir + 'meg/%s/%s-ica-raw.fif' %(subject,subject1), overwrite=True)
	del raw,ica
'''
...
event_id = dict(phonemic_in=15,infix_in=20,circumfix=35,morph_simple=5,prefix=30,compound=50,reduplicate=55,nonword=2)
...
subjects = ['P041','P043','P044','P045','P046']
...
for s in subjects:
	...
	if os.path.isfile(main_exp_dir + 'meg/%s/epoch selection/TagSurp_raw_epoch-man_corrected.pickled'%subject):
		rejfile = eelbrain.load.unpickle(main_exp_dir + 'meg/%s/epoch selection/TagSurp_raw_epoch-man_corrected.pickled' %subject)
	else:
		rejfile = eelbrain.load.unpickle(main_exp_dir + 'meg/%s/epoch selection/TagSurp_raw_epoch-man.pickled' %subject)
	...
	ev_circumfix = epochs['circumfix']
	ev_prefix = epochs['prefix']
	ev_compound = epochs['compound']
	ev_morph_simple = epochs['morph_simple']
	...
	ev_phonemic_in = epochs['phonemic_in'].average()
	ev_infix_in = epochs['infix_in'].average()
	ev_circumfix = epochs['circumfix'].average()
	ev_prefix = epochs['prefix'].average()
	ev_compound = epochs['compound'].average()
	ev_morph_simple = epochs['morph_simple'].average()
