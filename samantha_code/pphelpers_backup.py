...
def rawload(megpath, subj, expname, tag = '', preload = True):
    '''
    ...
    raw = mne.io.read_raw_fif('%s/%s/SAVANT/%s_%s%s-raw.fif' % (megpath, subj, subj, expname, tag), preload = preload)
...
def rawfilt(raw, h_freq = 40., l_freq = 0.1, plot_diff = True):
    ...
    if plot_diff:
        ...
        overwrite = 'y'
        if os.path.isfile('%s/%s/SAVANT/%s_%s_1-40-raw.fif' % (megpath, subj, subj, expname)):
            overwrite = input('Filtered file already exists. Overwrite?\nY/y = yes, N/n = no: ')
        if overwrite.lower() == 'y':
            raw.save('%s/%s/SAVANT/%s_%s_1-40-raw.fif' % (megpath, subj, subj, expname), overwrite = True)
...
def badsel(inst, redo = False):
    ...
    print('\nBAD CHANNELS')
    if (os.path.isfile('%s/%s/SAVANT/%s_%s-bad_channels.txt' % (megpath, subj, subj, expname)) or os.path.isfile('%s/%s/%s_%s-annot.fif' % (megpath, subj, subj, expname))) and redo == False:
        # Do you want to redo bad channels?
        with open('%s/%s/SAVANT/%s_%s-bad_channels.txt' % (megpath, subj, subj, expname)) as f:
        ...
        if overwrite.lower() == 'y':
            ...
            with open('%s/%s/SAVANT/%s_%s-bad_channels.txt' % (megpath, subj, subj, expname), 'w') as f:
...
def raw2epo(raw, tmin = -0.2, tmax = 0.6, baseline = (-0.1, 0.0)):
    '''
    ...
    mne.write_events('%s/%s/SAVANT/%s_%s_1-40-ica-eve.fif' % (megpath, subj, subj, expname), events)
    ...
    epochs.save('%s/%s/SAVANT/%s_%s_1-40-epo.fif' % (megpath, subj, subj, expname), overwrite = True)
...
def make_rej(epochs, show = True, save = False):
    ...
    if rejind == []:
        ...
        epochs_rej.save('%s/%s/SAVANT/%s_%s_1-40-ica-drop-epo.fif' % (megpath, subj, subj, expname), overwrite = True)
    ...
    if save == True:
        ...
        epochs.save('%s/%s/SAVANT/%s_%s_1-40-ica-rej-epo.fif' % (megpath, subj, subj, expname), overwrite = True)
        with open("%s/%s/SAVANT/%s_%s-rejected_epochs.txt" % (megpath, subj, subj, expname), "w") as text_file:
