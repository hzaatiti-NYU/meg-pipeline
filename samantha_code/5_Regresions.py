...
#subjects = ['Y0119', 'Y0208', 'Y0312', 'Y0321', 'Y0367', 'Y0368', 'Y0369', 'Y0371', 'Y0373', 'Y0374', 'Y0378', 'Y0379', 'Y0381', 'Y0387', 'Y0393', 'Y0395', 'Y0396']
print(len(subjects))
#subjects = ['Y0400']
...
output = os.path.join('new_baseline/savant_main', 'Plots/Group/%s%s_Regressions_n%s/' %(date,notes,str(sampleSize)))
...
#('M350 Analysis', (0.2, 0.5), ['LOBE.TEMPROAL-lh'], 'left temporal lobe', 'PALS_B12_Lobes', 'left', 'stemFrequency + wholeWordFreq', True, True, False),
    ...
    for subj in subjects:
        regResFileName = op.join('new_baseline/savant_main','Regressions/',subj,'Main',hemi,regionName,str(time[0])+'-'+str(time[1]),formula+'.pickle')
        if os.path.exists(regResFileName):
            ...
            if not os.path.exists(op.join('Logs/%s-Savant_Ara_NEW.log' %subj)):
                print('No log file for %s :(' %subj)
            else:
                #print('Found log file for %s' %subj)

                logFile = open(op.join('Logs','%s-Savant_Ara_NEW.log' %subj))
                founddict = {}
                ...
                for x in logFile:
                    if "!REJECTED!" in x:
                        print("contains rejected Epoch")
                        positionNo += 1
                    elif x.startswith("Picture\t") and ("hit" in x or "incorrect" in x or "miss" in x) and "practice;" not in x and "!REJECTED!" not in x:
...
#                                    print(wordCode)
                                    ...
                                    '''
                                    if onlyCorrect:
                                        if subj in ['Y0321','Y0367','Y0368','Y0369','Y0373','Y0374','Y0393','Y0382','Y0387','Y0378','Y0381','Y0379']:
                                            if "hit" not in correctOrNot:
                                                includeTrial = False
                                                #print(splitLine)
                                                #print("%s missed trial %s word %s" %(subj, str(positionNo), wordCode))
                                        else:
                                            if "hit" in correctOrNot: #because the log is coded backwards for these sujects
                                                includeTrial = False
                                                #print(splitLine)
                                                #print("%s missed trial %s word %s" %(subj, str(positionNo), wordCode))
                                    '''
                                    ...
                                    # we'll open the regressors file and get the variables that we want
                                        ...
                                        for y in csv.DictReader(regressorF):
                                            ...
                                            if wordCode == y['Word']:
                                                ...
                                                trProb = float(y['TP'])
                                                founddict[y['Word']] = 'found'
                                            else:
                                                if y['Word'] in founddict:
                                                    pass
                                                else:
                                                    founddict[y['Word']] = 1
                                                #print("not a match")
                                        ...
                                        if 'Gramm' in fileName:
                                            ###print(splitLine)
                                            if splitLine[1].split(';')[1] != ' 10':
                                                print("Stop! mismatch")
                                                print(splitLine[1].split(';')[1])
                                            else:
                                                condF.append('Gramm')
                ...
                logFile.close()
                fruits = "\n".join("{0} {1}".format(k, v)  for k,v in founddict.items())
                print(fruits)
            print(len(stcs))
            print('Loading fiff...')
            ...
            if not os.path.exists(op.join('new_baseline/savant_main','Regressions',subj,'Main',hemi,regionName,str(time[0])+'-'+str(time[1]))):
                os.makedirs(op.join('new_baseline/savant_main','Regressions',subj,'Main',hemi,regionName,str(time[0])+'-'+str(time[1])))
...
while my < imax:
    ...
    #print(imax)
        ...
        brainGraphName = op.join(output, 'graphs','%s_%s_cluster%s_%s-%s_%s-%s_%s_%s_p%s_brain.png' %(hemi,regionName,cluster_nb,str(tstart),str(tstop),str(p_start),str(p_stop),analysisType,coef,str(clusP)))
        if os.path.exists(graphName) and os.path.exists(brainGraphName):
        ...
        ########################################################################
            if plottimecourses:
                ...
                mne.write_labels_to_annot(labels,subject = 'fsaverage',subjects_dir=subjects_dir,
                    overwrite=True,
                    parc = coef + 'Cluster' + date + '_' + str(my)
                    )
